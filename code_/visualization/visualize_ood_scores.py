...
def plot_splits_scores(scores: Dict, scores_criteria: List[str], folder:Path=None) -> None:
    ...
    for score in scores_criteria:
        ...
        sns.lineplot(x=clusters, y=data_mean[score], marker="o", linewidth=2, color='blue', label=f"CO_{score.upper()} Scores")
        ...
        sns.lineplot(x=clusters, y=id_data_mean[score], marker="o", linewidth=2, color='orange', label=f"ID_{score.upper()} Scores")
...
def plot_splits_parity(predicted_values: dict,
                       ...
                       folder: Path) -> None:
    """
    Generate parity plots for each target starting with "CO_" based on predicted values and ground truth.
    ...
    # Loop over only the targets starting with 'CO_'
    for target in predicted_values.keys():
        if not target.startswith("CO_"):
            continue  # Skip targets that don't start with 'CO_'
        ...
        combined_data = pd.DataFrame({
            "True Values (nm)": true_values_ext,
            "Predicted Values (nm)": predicted_values_ext
        })
        
        # Calculate range for both axes and determine grid size
        ...
        # Get R² statistics
        r2_avg = score[target]["summary_stats"].get("test_r2_mean", 0)
        r2_stderr = score[target]["summary_stats"].get("test_r2_std", 0)
